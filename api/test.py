...
#解压路径
lptsrcdir = os.path.join(LPTPATH, 'src')

#可执行文件或者脚本
lptbindir = os.path.join(LPTPATH, 'bin')

#工具源码包
lpttoolsdir = os.path.join(LPTPATH, 'tools')

#results目录
lptresultsdir = os.path.join(LPTPATH, 'results')

#tmp
lpttmpdir = os.path.join(LPTPATH, 'tmp')

#data目录
lptdbdir = os.path.join(LPTPATH, 'db')
jobsxml = os.path.join(lptdbdir, 'jobs.xml')


#判断文件或者目录是否存在
for dir in (lptsrcdir, lptbindir, lpttoolsdir, lptresultsdir, lpttmpdir, lptdbdir):
