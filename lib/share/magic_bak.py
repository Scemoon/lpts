...
magic_database = [
    ...
    [0, 'string', '=', 'This is makeindex', 'Makeindex lptlog file'],
...
def guess_type_bak(filename):
    ...
    :return: Mimetype string or description, when appropriate mime not
            available.
    """
    lptlog.info(filename)
    if not os.path.isfile(filename):
        lptlog.debug('%s is not a file', filename)
        return None
    ...
    #for c in data:
    #    lptlog.info("guess_type-8")
    #    if ord(c) > 128:
    #        # Non ASCII (binary) data
    #        return 'Data'
    lptlog.info("guess_type-9")
    # ASCII, do some text tests
    #if str.find('The', data, 0, 8192) > -1:
    if data.find('The'.encode(), 0, 8192) > -1:
        lptlog.info('English text')
        return 'English text'
    #if str.find('def', data.decode('utf-8'), 0, 8192) > -1:
    if data.find('def'.encode(), 0, 8192) > -1:
        lptlog.info('Python Source')
        return 'Python Source'
    lptlog.info('ASCII text')
    return 'ASCII text'
